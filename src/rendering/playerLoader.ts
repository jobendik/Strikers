...
export interface PlayerModelData {
  /**
   * Normalised template group: scale already adjusted so character is ~1.7 m
   * tall with feet at y = 0.  Never add this directly to the scene — use
   * SkeletonUtils.clone() to produce an independent copy for each player.
   */
...
export async function loadPlayerModels(): Promise<void> {
  ...
  // Fix texture colour space and enable shadows on every mesh in the base model.
  base.traverse((obj) => {
    if (obj instanceof THREE.Mesh || obj instanceof THREE.SkinnedMesh) {
      obj.castShadow = true;
      obj.receiveShadow = false; // small perf saving — turf already has shadows
  ...
  // Normalise scale: make the character exactly 2.4 m tall with feet at y = 0.
  // (Slightly larger than real to look right from the overhead camera angle.)
  const rawBox = new THREE.Box3().setFromObject(base);
  const rawHeight = rawBox.max.y - rawBox.min.y;
  if (rawHeight > 0) {
    base.scale.multiplyScalar(2.4 / rawHeight);
  }
