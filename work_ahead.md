...
- `src/game/quests.ts` — daily orders/chest + weekly orders/activity (E2–E4).
- `src/game/season.ts` — Season Track engine (F1/F2): tier ladder + rewards, `progressSeason`, `claimTier`/`claimAll`, `unlockElite`. Extend the `TIERS` table / add a season rotation (A7) here; cosmetic ids seed the collection album (F3).
...
- **UI surfaces** (`index.html` + `src/style.css`): `src/ui/resultScreen.ts` (`#ft`, the Result Screen — D), `src/ui/profile.ts` (`#profileCard` — C4), `src/ui/commentary.ts` (`#commentary` — H1), `src/ui/hud.ts` (scoreboard, toasts, `showGoalFx`/`showScorerFlash`, half-time, shootout board; re-exports commentary), `src/ui/worldcup.ts` (`#tournament`), `src/ui/daily.ts` (daily card), `src/ui/weekly.ts` (weekly card), `src/ui/season.ts` (`#seasonCard` menu card + `#season` track screen — F1/F2).
- `src/platform/crazygames.ts` — CrazyGames SDK seam (B1–B4); `src/platform/storage.ts` — KV backend (B5).
- `src/main.ts` — boot wiring (init order: `initCrazyGames` (async, non-blocking) → UI buttons → result screen → settings → playerData → worldcup UI → daily card → weekly card → season card → profile card).
...
> **Progress snapshot (last updated this build):** the game is now **feature-complete across P0, P1 and P2** bar the genuinely browser/device/art-bound tail. On top of the prior spine (A1–A6 World Cup, B1–B5 SDK, C1–C4 progression/profile, D1–D4 Result Screen, E1–E4 daily/weekly, H1/H2 excitement) this build shipped the **entire economy + meta layer**: Season Track + Elite (F1/F2), cosmetic collection + shards + completion bonuses (F3/F4), free chests with published odds + pity (F5), an earned-currency daily shop (F6), in-match medals + achievements + nation mastery + hidden achievements (G1/G2/G3/G5), a difficulty ladder with XP multipliers (G4), the **rewarded-ad opt-in UI** (B4: double-XP + extra reroll), onboarding/loss/clean-exit (J1/J2/J3), claimable badges (I2), and the live-service layer — simulated leaderboard, rivals/feed, weekly events, region goals (K1–K4). Plus iOS audio unlock + SEO/share meta (L2/L3 code parts). **All cross-cutting guardrails hold (X1–X4):** ~280+ headless self-test assertions across the new engines, `npm run sim -- 16 180` green throughout (on-target ≤ shots, no NaN/guard; the two new difficulty tiers added to the gate), and **all of the new retention/economy/live code verified absent from the balance bundle — still 33 modules**. Honest economy throughout (published odds + pity, "items return", labelled simulations, opt-in ads that never gate progress).
>
> **What genuinely remains** (needs a real browser / devices / art tools — cannot be produced or verified headlessly): **L1** CrazyGames QA-tool pass, **L2** low-end-device FPS + touch ergonomics, **L3** thumbnail/screenshots/trailer, **I1** a wholesale RICOCHET visual redesign of the legacy menu/HUD, **H2** Best-Moment-of-the-Match capture, and **A7** post-WC longevity (reskins/classic tournaments/prestige). Everything built since #18 also still wants a **click-test + CrazyGames-QA pass** (it all builds clean — tsc + vite + sim green — but hasn't been run in a browser). See `docs/submission.md` for the launch checklist.
...
- [x] **B4** Rewarded ads (opt-in): double XP, reroll daily/weekly order. Honest UI. → engine `rewarded(onComplete)` (granted only on a fully-watched ad; no-op in dev so nothing is gated) **now wired to UI**: `src/ui/rewarded.ts` + a result-screen "DOUBLE THIS MATCH'S XP · watch a short ad" button (re-grants the match XP to account + season) and earned "EXTRA REROLL · ad" on the daily + weekly cards (`quests.rerollOrder/rerollWeeklyOrder` gained a non-spending `force`). Opt-in, never gates progress; honest framing wants final eyes-on the QA tool (L1).
...
- [ ] **L1** CrazyGames QA pass: SDK events fire, ads in QA tool, mute on blur, no external links, fast load. *(Browser/QA-tool only — can't be verified headlessly. Checklist in `docs/submission.md`.)*
- [~] **L2** Mobile perf + input ergonomics + iOS audio unlock; portrait rotate prompt. → iOS/mobile **audio unlock on first gesture** added (`main.ts`, respects the sound setting); LITE tier + portrait `#rotate` prompt already present. *(Low-end-device FPS + touch ergonomics passes need real devices.)*
- [~] **L3** Submission assets: thumbnail, screenshots, SEO, tags; IP/branding review. → SEO **keywords + Open Graph / Twitter meta** added; store copy + tags + IP rules drafted in `docs/submission.md`. *(Thumbnail/screenshots/trailer need a browser capture.)*
...
- [x] **F1** Free Season Track (~30 tiers, "World Cup 2026 Season") + reward claim flow. → `src/game/season.ts` (pure engine: 30-tier ladder w/ free+elite rewards, rising Season-XP curve, `progressSeason` roll-up capped at the top tier, battle-pass *claim* flow — nothing auto-grants) + reward-pipeline integration (`rewards.ts`: the same match XP advances the track; `MatchRewards.season`) + `src/ui/season.ts` (menu `#seasonCard` w/ tier + claim badge, full `#season` screen: per-tier free/elite cells with claim state, single-tap claim + CLAIM ALL) + result-screen season bar lit up (`modes.rewardBars`, green→gold) + boot wiring (`main.initSeasonCard`). Coins/gems/shards deposit to the wallet; cosmetic/title rewards deposit ids into `collection.owned` (seeds F3). 55-case headless self-test (curve, roll-up, cap, claim idempotency, retroactive elite, claimAll, rollover). *(UI wants an in-browser pass.)*
- [x] **F2** Earned Elite Track (weekly-order unlock) + retroactive unlock. → folded into the Season Track: each tier has an Elite reward column gated by `season.eliteUnlocked`; completing **all** of a week's weekly orders flips it on in the pipeline (`rewards.ts`, no payment — §3), surfaced as an `ELITE UNLOCKED` result-screen badge + chip. Unlock is **retroactive** by construction — every already-reached Elite tier becomes claimable at once (`tierState`). Claim state is stored additively in `season.claimed` (free = tier #, elite = `1000+tier`), so no schema bump. *(UI wants an in-browser pass.)*
- [x] **F3** Cosmetic collection album (kits, ball skins, celebrations, trails, banners, nets, stingers) + equip. → `src/game/collection.ts` (single catalogue w/ rarity + honest source, per-slot defaults, `equip`/`equippedIn`) + `src/ui/collection.ts` + `#collection` overlay + menu "vault" row w/ owned-count badge. Slot tabs, rarity-tinted grid, tap-to-equip. *(Equip is recorded state; applying ball/trail/net skins to the live render is a small browser-pass follow-up.)*
- [x] **F4** Shards + duplicate conversion + collection completion bonuses. → `grantCosmetic` converts duplicates to shards by rarity (5/15/40/120); `checkCompletionBonuses` pays once-each set bonuses (all-commons / all-balls / all-trails / full collection) via `flags`. 53-case self-test.
- [x] **F5** Free chests with visible odds + pity (epic≤10/mythic≤50); honest open. → `src/game/chests.ts` (Daily/Level-Up/Gold defs w/ published odds; `openChest` rolls under pity — Epic ≤10, Mythic ≤50 — pays coins + a cosmetic, dup→shards; RNG injectable) + earned from the daily-meter fill + level-ups (`rewards.ts`) + `src/ui/chest.ts` + `#chests` overlay w/ live pity + honest reveal. 15-case self-test.
- [x] **F6** Earned-currency shop, daily rotation, wishlist, no fake stock. → `src/game/shop.ts` (date-seeded deterministic shelf, cosmetics priced by rarity + repeatable chest/shard consumables; `buy` validates afford+ownership; owned cosmetics show OWNED; wishlist) + `src/ui/shop.ts` + `#shop` overlay ("items return — no fake stock"). 24-case self-test.
- [x] **G1** In-match medals (post-match-derivable set). → `src/game/medals.ts`: brace/hat-trick/avalanche/clean-sheet/no-sweat/marksman/wall/shootout-hero/world-champion, detected from facts + team stats in `modes.presentResult` (sim-safe), small coin bonus, tallied to `data.medals`, shown as result chips + in the Awards screen. *(Live-event medals — screamer/curler/wonder-save/last-minute — await an in-match capture hook; left out to stay sim-safe.)*
- [x] **G2** Achievements with visible partial progress + title/cosmetic/coin rewards. → `src/game/achievements.ts`: career-derived progress (wins/goals/clean-sheets/cups/collection/season/streak/medals), one-time grants, `#awards` screen w/ progress bars + menu badge. 25-case self-test (with G1/G3).
- [x] **G3** Nation mastery levels + "next mastery reward". → `src/game/mastery.ts`: per-nation XP + level curve + per-level coin rewards, granted for the nation played each match (`modes`), shown on the Awards "Mastery" tab.
- [x] **G4** Difficulty ladder additions (Ultimate/World Class) + XP multipliers. → `DIFF[3]/[4]` + extended `pen.*`/`FOV`/`RANGE` arrays + aim-noise floor; XP mult 0.9/1.0/1.15/1.3/1.5 in `rewards.ts` (result chip); menu buttons; **new tiers added to the sim regression gate** (no NaN/guard, on-target ≤ shots). *(Tier-specific medals not added.)*
- [~] **I1** Adopt RICOCHET design language across menu/HUD/result/profile. → Steadily adopted the tokens (neon/amber/violet/ice accents, card system, rarity tints, currency/badge pills) across all the new surfaces, but **not a wholesale redesign of the existing menu/HUD** — that's a visual pass best done with a browser. Left `[~]`.
- [~] **I2** Toast + medal-callout layers + claimable-reward badges on icon buttons. → Claimable/owned **badges** shipped on the vault buttons (collection/chests/awards counts), the season claim badge (pulsing), and the weekly-event banner. Medals/achievements reveal as result-screen chips. *(A dedicated slide-in medal-callout layer during play is browser-pass polish — left `[~]`.)*
- [x] **J1** Onboarding first-run flow + welcome tip overlay. → `src/ui/onboarding.ts` + `#intro` overlay (controls + honest progression promise + "simulated · no payments · no tracking" ribbon), shown once via `flags.seenIntro`.
- [x] **J2** Loss/comeback retention. → served by the result screen: XP always paid on a loss, "fought back" 2-star logic, honest next-best nudge, medals still earnable, the new opt-in **double-XP** comeback aid, and neutral copy (no shaming). *(Dramatic comeback medal awaits the live-event hook.)*
- [x] **J3** Clean exit/pause with progress + claimables + neutral copy. → `src/ui/pause.ts` + `#pauseInfo`: chest %, season tier, claimable rewards/chests, RESUME primary, "Progress saved · come back anytime." Decoupled from the gameplay path.

### P2 — live-service depth
- [x] **K1** Leaderboards (local, simulated) + participation tiers. → `src/game/rivals.ts` weekly leaderboard (`bumpWeeklyScore` resets each ISO week; player placed by real score; participation tier) + `#live` overlay. Labelled "simulated · local"; the CrazyGames leaderboard API slots in on-platform.
- [x] **K2** AI rivals + simulated activity river (honestly labelled). → fictional rival roster + deterministic "Simulation Feed" (`activityFeed`) shown in `#live` under a "not real players" header.
- [x] **K3** Rotating weekly events/modifiers. → `src/game/events.ts` (Double XP / Coin Rush / Goal Festival, deterministic per ISO week, each a **real** reward bonus) applied in `rewards.ts` (XP/coin mult + result chip) + a menu event banner. *(Gameplay-altering modifiers — tiny goals / one-life — left out to stay sim-safe.)*
- [x] **K4** Faction/region community goals (fiction-framed). → `regionGoal` in `rivals.ts`: a region community goal with simulated progress + the player's real career goals contributing, shown in `#live` under a "community simulation" label.
- [ ] **A7** Post-WC longevity: Euro/Copa reskins, classic "re-live history" tournaments, monthly seasons, prestige. *(Not built. The Season engine already supports rotation — `ensureSeason` resets on a new `SEASON_ID` — so monthly/seasonal rotation is a data change; the World Cup engine is field/group-count-driven for Euro/Copa reskins.)*
- [x] **G5** Hidden achievements (surprise rewards for unusual feats). → `achievements.ts` `hidden` flag (Goal Avalanche, Flawless); masked as "???" until earned, then revealed.

### Cross-cutting / always
- [x] **X1** Accessibility: reduced-motion + reduced-effects honoured. → result screen snaps to final state; CSS animations (claim-badge pulse, chest reveal) gated by `prefers-reduced-motion`; clear pause/exit with saved-progress copy; LITE tier.
- [x] **X2** Keep `npm run sim` green; on-target ≤ shots; no NaN/hang after every change. → green throughout this build (every cluster validated); the new difficulty tiers added to the gate; all retention/economy modules verified absent from the bundle (still 33 modules).
- [x] **X3** Ethical guardrails (§3) respected. → published chest odds + pity; shop "items return"/no fake stock; simulated leaderboard/feed/region clearly labelled; rewarded ads opt-in and never gate progress; first-run "simulated · no payments · no tracking" ribbon.
- [x] **X4** Update `work_ahead.md` as items ship. → done this build (no `memory/` dir in this repo; `docs/submission.md` added for the launch tail).
